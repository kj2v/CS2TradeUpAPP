...
struct Skin: Codable, Identifiable, Hashable {
    ...
    // ✅ 新增：定义缺失的属性，这样 ContentView 就可以直接访问了
    var stattrak: Bool?
...
// ✅ 新增：定义 CollectionInfo 结构体
struct CollectionInfo: Codable, Hashable, Identifiable {
    let id: String
    let name: String
    let image: String?
}
...
extension Color {
    init?(hex: String) {
        ...
        var rgb: UInt64 = 0
        let length = hexSanitized.count // 修复之前的 let/var 警告
